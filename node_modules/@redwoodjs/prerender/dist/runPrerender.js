...
const prerenderApolloClient = new _client.ApolloClient({
...
async function recursivelyRender(App, renderPath, gqlHandler, queryCache) {
  ...
  const componentAsHtml = _server.default.renderToString( /*#__PURE__*/(0, _jsxRuntime.jsx)(_router.LocationProvider, {
    location: {
      pathname: renderPath
    },
    children: /*#__PURE__*/(0, _jsxRuntime.jsx)(CellCacheContextProvider, {
...
function insertChunkLoadingScript(indexHtmlTree, renderPath, forVite) {
  ...
  if (!forVite) {
    ...
    const pagesIndex = (0, _indexOf.default)(_context4 = route.filePath).call(_context4, _path.default.join('web', 'src', 'pages')) + 8;
    const pagePath = (0, _projectConfig.ensurePosixPath)((0, _slice.default)(_context5 = route.filePath).call(_context5, pagesIndex));
  ...
  (0, _forEach.default)(chunkPaths).call(chunkPaths, chunkPath => {
    indexHtmlTree('head').prepend(`<script defer="defer" src="${chunkPath}" ${forVite ? 'type="module"' : ''}></script>`);
  });
  if (!forVite) {
    return;
  }
...
const runPrerender = async ({
  ...
  const {
    default: App
  } = require((0, _projectConfig.getPaths)().web.app);
  ...
  (0, _forEach.default)(_context6 = (0, _keys.default)(queryCache)).call(_context6, queryKey => {
    const {
      query,
      variables,
      data
  ...
  indexHtmlTree('head').append(`<script> globalThis.__REDWOOD__APOLLO_STATE = ${(0, _stringify.default)(prerenderApolloClient.extract())}</script>`);
