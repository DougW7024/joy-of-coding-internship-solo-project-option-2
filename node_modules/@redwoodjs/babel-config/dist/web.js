...
const getWebSideBabelPlugins = ({ forJest, forVite } = { forJest: false, forVite: false }) => {
  ...
  const commonPlugins = forVite ? [] : (0, import_common.getCommonPlugins)();
  const plugins = [
...
const getWebSideOverrides = ({ forPrerender, forVite } = {
  forPrerender: false,
  forVite: false
}) => {
  const overrides = [
...
const prebuildWebFile = (srcPath, flags = {}) => {
  const code = import_fs.default.readFileSync(srcPath, "utf-8");
