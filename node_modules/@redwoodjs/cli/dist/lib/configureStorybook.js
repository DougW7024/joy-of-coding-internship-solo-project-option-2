...
async function extendStorybookConfiguration(newConfigPath = void 0) {
  ...
  if (!import_fs_extra.default.existsSync(sbPreviewConfigPath)) {
    ...
    const storybookPreviewContent2 = ts ? templateContent : (0, import__.transformTSToJS)(sbPreviewConfigPath, templateContent);
    await (0, import__.writeFile)(sbPreviewConfigPath, storybookPreviewContent2);
  }
  const storybookPreviewContent = read(sbPreviewConfigPath);
  if (newConfigPath) {
    const newConfigTemplate = read(newConfigPath);
    const newConfigContent = ts ? newConfigTemplate : (0, import__.transformTSToJS)(newConfigPath, newConfigTemplate);
    const merged = (0, import_merge.merge)(storybookPreviewContent, newConfigContent, {
    ...
    const formatted = import_prettier.default.format(merged, {
      parser: "babel",
      ...await import_prettier.default.resolveConfig(sbPreviewConfigPath)
    });
