...
`,
  ...
  const webCommand = (0, import_project_config.getConfig)().web.bundler !== "webpack" ? `yarn cross-env NODE_ENV=development rw-vite-dev ${forward}` : `yarn cross-env NODE_ENV=development RWJS_WATCH_NODE_MODULES=${watchNodeModules ? "1" : ""} webpack serve --config "${webpackDevConfig}" ${forward}`;
  const jobs = {
    api: {
      name: "api",
      command: [
