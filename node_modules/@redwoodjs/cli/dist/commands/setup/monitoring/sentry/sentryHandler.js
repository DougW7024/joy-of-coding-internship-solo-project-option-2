...
const handler = async ({ force }) => {
  ...
  const tasks = new import_listr2.Listr([
    ...
    },
    {
      ...
      task: async (ctx) => {
        ...
        const handlerIndex = contentLines.findLastIndex(
          (line) => /^export const handler = createGraphQLHandler\({/.test(line)
        );
        const pluginsIndex = contentLines.findLastIndex(
          (line) => /extraPlugins:/.test(line)
        );
    ...
    },
    {
      ...
      task: async () => {
        ...
        const boundaryOpenIndex = contentLines.findLastIndex(
          (line) => /<FatalErrorBoundary page={FatalErrorPage}>/.test(line)
        );
        contentLines.splice(
          boundaryOpenIndex,
          1,
        ...
        const boundaryCloseIndex = contentLines.findLastIndex(
          (line) => /<\/FatalErrorBoundary>/.test(line)
        );
    ...
    },
    {
      ...
      task: (ctx) => {
        notes.push(
          import_cli_helpers.colors.green(
