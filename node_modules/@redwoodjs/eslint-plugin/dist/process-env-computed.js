...
function isProcessEnv(node) {
  return isMemberExpression(node) && hasName(node.object, "process") && hasName(node.property, "env");
}
function isIdentifier(node) {
  return typeof node !== "undefined" && node.type === "Identifier";
}
function isMemberExpression(node) {
  return typeof node !== "undefined" && node.type === "MemberExpression";
}
  ...
  create(context) {
    return {
      MemberExpression: function(node) {
        if (isProcessEnv(node.object) && node.computed && !isTestFile(context.filename)) {
          context.report({
            message: "Accessing process.env via array syntax will break in production. Use dot notation e.g. process.env.MY_ENV_VAR instead",
            node
            // fix(fixer) {},
          });
        }
